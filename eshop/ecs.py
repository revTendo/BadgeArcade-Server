...
_ESHOP_DIR = os.path.dirname(os.path.abspath(__file__))
...
def soap():
    ...
    if 'ecs:AccountGetETickets' in parsed['SOAP-ENV:Envelope']['SOAP-ENV:Body']:
        ...
        if ds.devicecert_consoleid == None:
            ds.devicecert_consoleid = format(int(ds.consoleid) & 0xFFFFFFFF, '08X')
            ds.save()
        print("Generating ticket...")
        if istitle:
            with open(os.path.join(_ESHOP_DIR, "basetik.bin"), "rb") as f:
                ticket_bytes = f.read()
        else:
            with open(os.path.join(_ESHOP_DIR, "basetik_licence.bin"), "rb") as f:
                ticket_bytes = f.read()
        if istitle:
        ...
        with BytesIO(ticket_bytes) as tk:
            ...
            tk.seek(0x1D8)
            tk.write(int.to_bytes(int(ds.consoleid) & 0xFFFFFFFF, 4, "big"))
    ...
    if 'ecs:GetTaxes' in parsed['SOAP-ENV:Envelope']['SOAP-ENV:Body']:
        try:
            ...
            return "Error"
        items_val = parsed['SOAP-ENV:Envelope']['SOAP-ENV:Body']['ecs:GetTaxes']['ecs:Items']
        if isinstance(items_val, list):
            items_val = items_val[0] if items_val else ""
        items_val = str(items_val).strip()
        if items_val in ("28", "29"):
            r = make_response(render_template("ecs/getTaxes.xml", id=ds.consoleid, message=parsed['SOAP-ENV:Envelope']['SOAP-ENV:Body']['ecs:GetTaxes']['ecs:MessageId'], time=int(round(time.time()*1000)), country=ds.country, itemid=items_val, itemprice="1.00"))
            r.headers.set("Content-Type", "text/xml; charset=utf-8")
            return r
        try:
            aitem = item.objects.get(id=items_val)
        except ObjectDoesNotExist:
            return "Error"
