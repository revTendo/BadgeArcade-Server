...
def _region_ids(region_token):
    """Map the eShop URL region token (US/EU/JP...) to the set of shopdeckdb_region ids
    whose 'initial' falls in that bucket. Titles with region=NULL are hidden from everyone."""
    token = (region_token or "").upper()
    if token in ("EU", "EUR", "EUROPE"):
        needles = ("EUR",)
    elif token in ("JP", "JPN", "JAPAN"):
        needles = ("JPN",)
    else:
        needles = ("USA", "US")
    ids = []
    for r in region.objects.all():
        init = (r.initial or "").upper()
        if any(n in init for n in needles):
            ids.append(r.id)
    return ids
...
def directory(request, region, cid):
    ...
    if request.GET.get("platform[]") == None and request.GET.get("genre[]") == None and request.GET.get("publisher[]") == None and request.GET.get("price_max")==None and request.GET.get("price_min")==None:
        titles = Title.objects.filter(category=dir, public=True, region_id__in=_region_ids(region)).order_by('-date')[int(request.GET.get("offset")):25+25]
        total = titles.count()
        ...
        if request.GET.get("price_max") != None and request.GET.get("price_min") == None:
            titles = Title.objects.filter(price__lte=int(request.GET.get("price_max")),category=dir, platform__in=platforms, genre__in=genrel, publisher__in=publisherl, public=True, region_id__in=_region_ids(region)).distinct().order_by("-date")[int(request.GET.get("offset")):25+25]
        if request.GET.get("price_min") != None and request.GET.get("price_max") == None:
            titles = Title.objects.filter(price__gte=int(request.GET.get("price_min")),category=dir, platform__in=platforms, genre__in=genrel, publisher__in=publisherl, public=True, region_id__in=_region_ids(region)).distinct().order_by("-date")[int(request.GET.get("offset")):25+25]
        if request.GET.get("price_min") != None and request.GET.get("price_max") == None:
            titles = Title.objects.filter(price__gte=int(request.GET.get("price_min")),price__lte=int(request.GET.get("price_max")),category=dir, platform__in=platforms, genre__in=genrel, publisher__in=publisherl, public=True, region_id__in=_region_ids(region)).distinct().order_by("-date")[int(request.GET.get("offset")):25+25]
        if request.GET.get("price_max") == None and request.GET.get("price_min")==None:
            titles = Title.objects.filter(category=dir, platform__in=platforms, genre__in=genrel, publisher__in=publisherl, public=True, region_id__in=_region_ids(region)).distinct().order_by("-date")[int(request.GET.get("offset")):25+25]
        total = titles.count()
    alltitles = []
    i = 0
    for title in titles:
        ...
        if title.demo != None:
            ...
            demo = False
        alltitles.append({"title": {"platform": {"name": title.platform.name, "id": title.platform.id, "device": "CTR", "category": (_first_genre(title).id if _first_genre(title) else 0)}, "publisher": {"name": title.publisher.publisher_name, "id": title.publisher.id}, "display_genre": (_first_genre(title).name if _first_genre(title) else ""), "release_date_on_eshop": str(title.date), "retail_sales": False, "eshop_sales": is_downloadable, "demo_available": demo, "aoc_available": False, "in_app_purchase": title.in_app_purchase, "release_date_on_original": str(title.date), "name": title.name, "id": title.id, "product_code": title.product_code, "icon_url": title.icon_url, "banner_url": title.banner_url, "new": title.new}, "index": i})
        i = i + 1
    try:
        for Movie in movies:
            if i > 25:
...
def title(request, region, tid):
    try:
        title = Title.objects.get(id=tid, public=True, region_id__in=_region_ids(region))
    except ObjectDoesNotExist:
...
def contents(request, region):
    ...
    all_titles = Title.objects.filter(name__icontains=search_term, public=True, region_id__in=_region_ids(region)).order_by("-date")[int(request.GET.get("offset")):25+25]
    ...
    for title in all_titles:
        ...
        if title.demo != None:
            ...
            demo = False
        titles.append({"title": {"platform": {"name": title.platform.name, "id": title.platform.id, "device": "CTR", "category": (_first_genre(title).id if _first_genre(title) else 0)}, "publisher": {"name": title.publisher.publisher_name, "id": title.publisher.id}, "display_genre": (_first_genre(title).name if _first_genre(title) else ""), "release_date_on_eshop": str(title.date), "retail_sales": False, "eshop_sales": is_downloadable, "demo_available": demo, "aoc_available": False, "in_app_purchase": title.in_app_purchase, "release_date_on_original": str(title.date), "name": title.name, "id": title.id, "product_code": title.product_code, "icon_url": title.icon_url, "banner_url": title.banner_url, "new": title.new}, "index": i})
        i = i + 1
    for Movie in movies:
        if i > 25:
            continue
...
def titles(request, region):
    if request.GET.get("platform[]") == None and request.GET.get("genre[]") == None and request.GET.get("publisher[]") == None and request.GET.get("price_max")==None and request.GET.get("price_min")==None and request.GET.get("title[]") == None:
        ...
        if request.GET.get('title[]') == None:
            all_titles = Title.objects.filter(public=True, region_id__in=_region_ids(region)).order_by("-date")[int(request.GET.get("offset")):25+25]
            if request.GET.get("release_date_after") == None:
    ...
    if request.GET.get("platform[]") != None or request.GET.get("genre[]") != None or request.GET.get("publisher[]") != None or request.GET.get("price_max")!=None or request.GET.get("price_min")!=None:
        ...
        if request.GET.get("freeword") != None:
            search_term = unquote(request.GET.get("freeword"))
            if request.GET.get("price_max") != None and request.GET.get("price_min") == None:
                all_titles = Title.objects.filter(price__lte=int(request.GET.get("price_max")), name__icontains=search_term,platform__in=platforms, genre__in=genrel, publisher__in=publisherl, public=True, region_id__in=_region_ids(region)).distinct().order_by("-date")[int(request.GET.get("offset")):25+25]
            if request.GET.get("price_min") != None and request.GET.get("price_max") == None:
                all_titles = Title.objects.filter(price__gte=int(request.GET.get("price_min")),name__icontains=search_term,platform__in=platforms, genre__in=genrel, publisher__in=publisherl, public=True, region_id__in=_region_ids(region)).distinct().order_by("-date")[int(request.GET.get("offset")):25+25]
            if request.GET.get("price_max") != None and request.GET.get("price_min") != None:
                all_titles = Title.objects.filter(price__lte=int(request.GET.get("price_max")),price__gte=int(request.GET.get("price_min")),name__icontains=search_term,platform__in=platforms, genre__in=genrel, publisher__in=publisherl, public=True, region_id__in=_region_ids(region)).distinct().order_by("-date")[int(request.GET.get("offset")):25+25]
            if request.GET.get("price_max") == None and request.GET.get("price_min")==None:
                all_titles = Title.objects.filter(name__icontains=search_term,platform__in=platforms, genre__in=genrel, publisher__in=publisherl, public=True, region_id__in=_region_ids(region)).distinct().order_by("-date")[int(request.GET.get("offset")):25+25]
        else:
            if request.GET.get("price_max") != None and request.GET.get("price_min") == None:
                all_titles = Title.objects.filter(price__lte=int(request.GET.get("price_max")), platform__in=platforms, genre__in=genrel, publisher__in=publisherl, public=True, region_id__in=_region_ids(region)).distinct().order_by("-date")[int(request.GET.get("offset")):25+25]
            if request.GET.get("price_min") != None and request.GET.get("price_max") == None:
                all_titles = Title.objects.filter(price__gte=int(request.GET.get("price_min")), platform__in=platforms, genre__in=genrel, publisher__in=publisherl, public=True, region_id__in=_region_ids(region)).distinct().order_by("-date")[int(request.GET.get("offset")):25+25]
            if request.GET.get("price_max") != None and request.GET.get("price_min") != None:
                all_titles = Title.objects.filter(price__lte=int(request.GET.get("price_max")),price__gte=int(request.GET.get("price_min")),platform__in=platforms, genre__in=genrel, publisher__in=publisherl, public=True, region_id__in=_region_ids(region)).distinct().order_by("-date")[int(request.GET.get("offset")):25+25]
            if request.GET.get("price_max") == None and request.GET.get("price_min")==None:
                all_titles = Title.objects.filter(platform__in=platforms, genre__in=genrel, publisher__in=publisherl, public=True, region_id__in=_region_ids(region)).distinct().order_by("-date")[int(request.GET.get("offset")):25+25]
        total = all_titles.count()
    if request.GET.get("title[]") != None:
    ...
    for i, title in enumerate(titles_with_votes, start=1):
        ...
        ranking_list.append({
            "title": {
                ...
                "name": "• " + (title.region.initial if title.region else "") + " • " + "\n" + title.name,
                "id": title.id,
