...
class ParentalControlAdmin(admin.ModelAdmin):
    ...
    list_display = ("id", "parental_system_name", "age_name", "age_number")
    search_fields = ("parental_system_name", "age_name")
...
class TitleAdminForm(forms.ModelForm):
    add_all_eu = forms.BooleanField(required=False, label="Add all EU countries")
    add_all_us = forms.BooleanField(required=False, label="Add all US/Americas countries")
    add_all_jp = forms.BooleanField(required=False, label="Add all JP countries")
...
class TitleAdmin(admin.ModelAdmin):
    ...
    def save_related(self, request, form, formsets, change):
        ...
        picks = []
        if form.cleaned_data.get("add_all_eu"):
            picks.append("EU")
        if form.cleaned_data.get("add_all_us"):
            picks.append("US")
        if form.cleaned_data.get("add_all_jp"):
            picks.append("JP")
        if picks:
            form.instance.countries.add(*countryCode.objects.filter(group__code__in=picks))
    ...
    autocomplete_fields = ("publisher", "platform", "demo", "parentalControl")
    filter_horizontal = ("genre", "countries", "category")
...
@admin.register(ratingDescriptor)
class RatingDescriptorAdmin(admin.ModelAdmin):
    list_display = ("id", "name")
    search_fields = ("name",)
