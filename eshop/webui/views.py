...
def title(request, tid):
    ...
    return render(request, "title.html", {"title": title.name, "WEBUI_NAME":WEBUI_NAME, "user": request.user, "app": title, "updates": updates, "wishlisted": wishlisted, "screenshots": []})
...
def titles(request):
    ...
    updates = len(titles_to_update)
    all_titles = Title.objects.filter(public=True).order_by('-date')
    return render(request, "titles.html", {"title": "All Titles", "WEBUI_NAME": WEBUI_NAME, "user": request.user, "updates": updates, "results": all_titles})
...
def news(request):
    ...
    messages = motd.objects.all().order_by('order')
    return render(request, "news.html", {"title": "News", "WEBUI_NAME": WEBUI_NAME, "user": request.user, "updates": updates, "announcements": announcements, "messages": messages})
...
def search(request):
    ...
    if not request.GET.get("query"):
        ...
        if not query:
            return render(request, "_error.html", {"title": "Error", "WEBUI_NAME":WEBUI_NAME, "user": request.user,"updates": updates, "message": "Nothing was put as the title name."})
        searched = Title.objects.filter(name__icontains=query, public=True).order_by('-date')
        return render(request, "searchresult.html", {"title": "Results for "+query, "WEBUI_NAME": WEBUI_NAME, "user": request.user, "updates": updates, "results": searched, "query": query})
